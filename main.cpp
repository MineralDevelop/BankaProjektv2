...
int main()
{
    ...
    if(odabirtransakcije=='a') // UPLATA
    ...
    else if(odabirtransakcije=='b') /// ISPLATA
      {
        ...
        for(i=1;i>0;i++)
          ...
          {
            cout<<"\n Novce sa racuna prebacujete na karticu\n Unesite svotu novca koju zelite isplatiti (EUR): ";
              for(i=1;i>0;i++)
                {
                cin>>kartica;
                printf("\n\n Odaberite valutu u kojoj zelite isplatiti novac: a) Euro (%.2f EUR)\n\t\t\t\t\t\t\t\t\t\t\t\t  b) Funte (%.2f GPB)\n\t\t\t\t\t\t\t\t\t\t\t\t  c) Americki dolar (%.2f USD)\n\t\t\t\t\t\t\t\t\t\t\t\t  d) Poljski zlot (%.2f PLN)\n\t\t\t\t\t\t\t\t\t\t\t\t  e) Mađarska forinta (%.2f HUF)\n\t\t\t\t\t\t\t\t\t\t\t\t  f) Srpski dinar (%.2f RSD)\n\t\t\t\t\t\t\t\t\t\t\t\t  g) Svicarski franak (%.2f CHF)\n\t\t\t\t\t\t\t\t\t\t\t\t  h) Rumunjski lej (%.2f RON)\n\t\t\t\t\t\t\t\t\t\t\t\t  i) Konvertibilna marka (%.2f KM)\n\t\t\t\t\t\t\t\t\t\t\t\t  j) Povratak\n\n\t\t\t\t\t\t\t\t\t\t  Odabir: ",kartica,kartica*0.86,kartica*1.08,kartica*4.52,kartica*371.02,kartica*117.22,kartica*0.98,kartica*4.97,kartica*1.96);
               for(i=1;i>0;i++)
              {
                iskoristeno=1;
                cin>>valuta;
  ...
  crypto.close();      
    ...
    else if(odabirtransakcije=='e') /// PREBACIVANJE
      {
          ...
          for(int k=brojkica-1; k>=0; k--)
            {
              ...
              if(OIBzaPrebacivanje==RezervaOIBM[k])
              {
                ...
                while(1)
                {
                  ...
                  if(stanjeracuna[klijenti]-SvotaZaPrebacivanje<0)
                  {
                    cout<<"\n Ne možete ići u minus, pokušajte ponovno: ";
                  }
                  else
